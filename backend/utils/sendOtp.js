...
export const sendOtpEmail = async (email, otp) => {
  ...
  if (!user || user === "mock-email@gmail.com" || !pass || pass === "mock-pass") {
    if (process.env.NODE_ENV !== "production") {
      console.log(`[Dev Mock OTP] OTP for ${email} generated successfully.`);
    }
    return true;
  }
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; color: #1e1b4b; background: #fafafd; border-radius: 12px; border: 1px solid rgba(99, 102, 241, 0.15); max-width: 500px; margin: auto;">
        ...
        <p>You requested a verification code for your BigO.ai account.</p>
        <div style="font-size: 28px; font-weight: bold; background: #f0f2ff; padding: 15px; text-align: center; border-radius: 8px; letter-spacing: 6px; color: #4f46e5; margin: 20px 0;">
          ${otp}
        </div>
        ...
        <p style="font-size: 11px; text-align: center; color: #94a3b8;">© ${new Date().getFullYear()} BigO.ai. All rights reserved.</p>
      </div>
    `,
  };
